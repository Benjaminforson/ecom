...
function productSoldQtyByProductId($con,$pid){
	$sql="select sum(order_detail.qty) as qty from order_detail,`order` where `order`.id=order_detail.order_id and order_detail.product_id=$pid and `order`.order_status!=4 and ((`order`.payment_type='MTN' and `order`.payment_status!='') or (`order`.payment_type='VODAFONE' and `order`.payment_status!='') or (`order`.payment_type='AIRTELTIGO' and `order`.payment_status!=''))";
	$res=mysqli_query($con,$sql);
